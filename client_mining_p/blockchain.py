...
class Blockchain(object):
    ...
    def hash(self, block):
        ...
        # We want hexidecimal format because it's easier to work with
...
def mine():
    ...
    required = ["proof", "id"]
    if not all (k in server_data for k in required):
        response = {
    ...
    submitted_proof = server_data.get("proof")
    last_block = blockchain.last_block
    last_block_string = json.dumps(last_block, sort_keys=True)

    if blockchain.valid_proof(last_block_string, submitted_proof):
        prev_hash = blockchain.hash(last_block)
        block = blockchain.new_block(submitted_proof, prev_hash)
